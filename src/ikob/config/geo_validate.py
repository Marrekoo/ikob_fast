...
ikob.config.validate.FileValidator directly -- but is called from it
(see FileValidator.validate_input_files in ikob/config/validate.py).
...
def _validate_segs_gpkg(config) -> bool:
    from ikob import geo_utils, segs_mapping
    ...
    if not complete:
        logger.error(
            "SEGS column mapping is incomplete; still missing: %s. "
            "Finish the 'Zonedata' step (ikob.ikobwizard) or edit "
            "project.paden.segs_mapping directly.", missing,
        )
        return False

    logger.info("SEGS GeoPackage %s: reference layer '%s' OK, mapping complete.", gpkg_path, buurten_layer)
    return True
